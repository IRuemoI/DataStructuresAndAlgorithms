...
{
    //	private static Max(int[] cands, int[] sroces) {
    ...
    private static int F(int[] cands, int[] sroces, int index, int hold, int cur, int next)
    {
        ...
        var p1 = -1;
        if (all1 > sroces[index]) p1 = F(cands, sroces, index + 1, hold + cands[index], next, 0);
        // 爆发
        var all2 = hold + cur + cands[index] * 3;
        var p2 = -1;
        if (all2 > sroces[index]) p2 = F(cands, sroces, index + 1, hold, next + cands[index] * 3, cands[index] * 3);
    ...
    private static int Process(int[] cards, int[] scores, int index, int hold, int cur, int next)
    {
        ...
        var p1 = -1;
        if (d1 > scores[index]) p1 = Process(cards, scores, index + 1, hold + cards[index], next, 0);
        var d2 = hold + cur + cards[index] * 3;
        var p2 = -1;
        if (d2 > scores[index]) p2 = Process(cards, scores, index + 1, hold, next + cards[index] * 3, cards[index] * 3);
        return Math.Max(p1, p2);
    }
    ...
    private static int P(int[] cands, int[] sroces, int index, int hold, int cur, int next)
    {
        ...
        var p2 = -1;
        if (all2 > sroces[index]) p2 = F(cands, sroces, index + 1, hold, next + cands[index], cands[index]);
        return Math.Max(p1, p2);
    }
