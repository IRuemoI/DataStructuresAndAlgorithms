...
public static class BellmanFord
{
    // Bellman-Ford算法
    // 针对此题改写了松弛逻辑，课上讲了细节
    private static int FindCheapestPrice(int n, int[][] flights, int start, int target, int k)
    {
        var cur = new int[n];
        Array.Fill(cur, int.MaxValue);
        cur[start] = 0;
        for (var i = 0; i <= k; i++)
        {
            var next = new int[n];
            ...
            foreach (var edge in flights)
                // a -> b , w
                if (cur[edge[0]] != int.MaxValue)
                    next[edge[1]] = Math.Min(next[edge[1]], cur[edge[0]] + edge[2]);

            cur = next;
        }

        return cur[target] == int.MaxValue ? -1 : cur[target];
    }
}