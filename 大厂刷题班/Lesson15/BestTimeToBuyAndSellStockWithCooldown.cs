...
public class BestTimeToBuyAndSellStockWithCooldown
{
    ...
    private static int Process1(int[] prices, bool buy, int index, int buyPrices)
    {
        if (index >= prices.Length) return 0;
        if (buy)
        {
            var noSell = Process1(prices, true, index + 1, buyPrices);
            var yesSell = prices[index] - buyPrices + Process1(prices, false, index + 2, 0);
            return Math.Max(noSell, yesSell);
        }

        var noBuy = Process1(prices, false, index + 1, 0);
        var yesBuy = Process1(prices, true, index + 1, prices[index]);
    ...
    {
        if (prices.Length < 2) return 0;
        var buy1 = Math.Max(-prices[0], -prices[1]);
        var sell1 = Math.Max(0, prices[1] - prices[0]);
        var sell2 = 0;
    ...
    {
        Console.WriteLine(MaxProfit1([1, 2, 3, 0, 2])); //输出3
        Console.WriteLine(MaxProfit2([1, 2, 3, 0, 2])); //输出3
        Console.WriteLine(MaxProfit3([1, 2, 3, 0, 2])); //输出3
    }
}